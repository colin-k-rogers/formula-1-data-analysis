...
# This script owns VPC_ID/SUBNET_ID/SG_ID/APP_ID only -- STAGING_APP_ID
# belongs to ./setup_staging.sh. Capture it from whatever's already in
# state.sh (if anything) before the heredoc below overwrites the whole
# file, so rerunning this script (e.g. after pushing a new production
# image) doesn't silently erase staging setup that's already been done.
EXISTING_STAGING_APP_ID=""
if [ -f ./state.sh ]; then
    EXISTING_STAGING_APP_ID=$(. ./state.sh; echo "${STAGING_APP_ID:-}")
fi
...
EOF
if [ -n "$EXISTING_STAGING_APP_ID" ]; then
    echo "export STAGING_APP_ID=\"${EXISTING_STAGING_APP_ID}\"" >> ./state.sh
fi
